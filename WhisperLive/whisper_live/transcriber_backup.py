...
class WhisperModel:
    def __init__(
        ...
        with_language_classifier: bool = True,  #新增一行: 是否使用自定义语言分类器
    ):
        ...
        self.with_language_classifier = with_language_classifier
        if with_language_classifier:
            ...
            self.language_classifier.load_state_dict(
                torch.load("/root/ASR_TTS_improvement/models/language_fnn_only.pt", map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
            )
    ...
    def transcribe(                                                         # noqa: C901
            ...
            if language is None:
                if not self.model.is_multilingual:
                    ...
                    ###删除386行内容，新增是否使用自定义识别逻辑 （389-398行）
                    if self.with_language_classifier:
                        if use_custom_language_classifier:
                            print("Using custom language classifier")
                            results = self.custom_detect_language(encoder_output)[0]
                        else:
                            results = self.model.detect_language(encoder_output)[0]
                    else:
                        if use_custom_language_classifier:
                            print("Custom language classifier not loaded, using default")
                        results = self.model.detect_language(encoder_output)[0]
