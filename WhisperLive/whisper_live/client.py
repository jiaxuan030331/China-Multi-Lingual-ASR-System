...
class Client:
    ...
    def process_segments(self, segments):
        ...
        # Concatenate the segments into final_transcript
         # Concatenate the segments into final_transcript
        
        # Truncate to last 3 entries for brevity.
        text = text[-3:]
        utils.clear_screen()
        self.transcript_text = text="".join(text)
...
class TranscriptionTeeClient:
    ...
    """
    def __init__(self, clients, save_output_recording=False, output_recording_filename="./output_recording.wav",vad_sensitivity=3, vad_timeout=2.0):
    ...
    def is_speech(self, audio_chunk):
        """使用 VAD 检测音频是否包含语音"""
        # 确保 audio_chunk 的长度为 10ms 的整数倍
        frame_duration = 30  # 可以是 10, 20 或 30
        frame_length = int(self.rate * frame_duration / 1000)
        audio_chunk = audio_chunk[:frame_length * 2]  # 16-bit PCM, 每个样本 2 字节
        return self.vad.is_speech(audio_chunk, sample_rate=self.rate)
        ...
        try:
            for _ in range(0, int(self.rate / self.chunk * self.record_seconds)):
                ...
                if self.is_speech(data):
                    self.last_active_time = time.time()
                elif time.time() - self.last_active_time > self.vad_timeout:
                    print("[INFO]: No speech detected, stopping recording.")
                    break
...
class TranscriptionClient(TranscriptionTeeClient):
    ...
    def get_transcript_text(self):
        return self.client.transcript_text.strip()
