...
class WhisperModel:
    def __init__(
        ...
        if language_classifier_path:
            ...
            self.language_classifier.load_state_dict(
    ...
    def transcribe(                                                         # noqa: C901
            ...
            #print(f"[⏱] decode_audio took {time.time() - start:.3f}s")
            ...
            if vad_filter:
                ...
                #print(f"[DEBUG] Audio after VAD duration: {duration_after_vad:.2f}s")
                self.logger.info(
                ...
                if not self.model.is_multilingual:
                    ...
                    if self.language_classifier:
                        if use_custom_language_classifier:
                            ...
                            results = self.custom_detect_language(encoder_output)[0]
                        else:
                            print("Using default language classifier, dectected as: ",end="")
                            results = self.model.detect_language(encoder_output)[0]
                    else:
                        if use_custom_language_classifier:
                            print("Custom language classifier not loaded, using default language classifier, dectected as: ",end="")
                        results = self.model.detect_language(encoder_output)[0]
                    logging.info("language result:")
            ...
            segments = self.generate_segments(features, tokenizer, options, encoder_output = None)
            
            #print(f"[⏱] generate_segments took {time.time() - start:.3f}s")

            if speech_chunks:
                #start = time.time()
                segments = restore_speech_timestamps(segments, speech_chunks, sampling_rate)
                #print(f"[⏱] restore_speech_timestamps took {time.time() - start:.3f}s")
            ...
            #print(f"[✅⏱] Total transcription time: {time.time() - start_total:.3f}s")
    ...
    def generate_segments(
            ...
            while clip_idx < len(seek_clips):
                #(f"\n[DEBUG] Loop Entry: clip_idx={clip_idx}, seek={seek}, seek_clips={seek_clips}, content_frames={content_frames}")

                seek_clip_start, seek_clip_end = seek_clips[clip_idx]
                #print(f"[DEBUG] seek_clip_start={seek_clip_start}, seek_clip_end={seek_clip_end}")

                if seek_clip_end > content_frames:
                    seek_clip_end = content_frames
                    #print(f"[DEBUG] seek_clip_end clipped to content_frames={content_frames}")

                if seek < seek_clip_start:
                    #print(f"[DEBUG] seek < seek_clip_start: updating seek from {seek} to {seek_clip_start}")
                    seek = seek_clip_start

                if seek >= seek_clip_end:
                    #print(f"[DEBUG] seek >= seek_clip_end ({seek} >= {seek_clip_end}), moving to next clip")
                    clip_idx += 1
                    if clip_idx < len(seek_clips):
                        seek = seek_clips[clip_idx][0]
                    continue

            # If it passes above checks, we're inside processing block
                #print(f"[DEBUG] ✅ Entering segment processing: seek={seek}, segment window end={(seek + self.feature_extractor.nb_max_frames)}")                
    ...
    def custom_detect_language(self, encoder_output) -> List[List[Tuple[str, float]]]:
        """
        新增自定义识别函数，使用自定义 FNN 对 encoder_output 进行语言预测，返回与原生 detect_language 相同结构。
        
        Args:
            encoder_output: ctranslate2.StorageView 类型 (1, T, H)
        
        Returns:
            List[List[Tuple[str, float]]]: 例如 [[("<|zh|>", 0.9), ("<|en|>", 0.1), ...]]
        """
